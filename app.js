...
const traumatologia = [
    ...
    { hora: '9:00', especialista: 'RENÉ POBLETE', paciente: 'PANA GELLONA', rut: '13123329-7', prevision: 'ISAPRE' },
...
//elimina primer y ultimo elemento radiologia
const nuevaRadiologia = radiologia.slice(1, radiologia.length - 1)
console.log({nuevaRadiologia});

//lista de pacientes dental separados por guion
dental.forEach(paciente => {
    document.write(`${paciente.hora} - ${paciente.paciente} - ${paciente.especialista} - ${paciente.rut} - ${paciente.prevision} <br />`);
})

//lista todos los pacientes
const todosLosPacientes = radiologia.concat(dental, traumatologia)

todosLosPacientes.forEach(paciente => {
    document.write(`${paciente.paciente} <br />`);
})

//filtro pacientes isapre dental
const pacientesFiltradosIsapre = dental.filter(paciente => {
    if (paciente.prevision === "ISAPRE") {
        return paciente;
    }
})

console.log({pacientesFiltradosIsapre})

//filtro pacientes Fonasa Traumatologia
const pacientesFiltradosFonasa = traumatologia.filter(paciente => {
    if (paciente.prevision === "FONASA") {
        return paciente;
    }
})

console.log({pacientesFiltradosFonasa})
